...
object UpdateUtils {
    ...
    private const val KEY_ADDRESSBOOK_URL = "addressbook_url"

    class LoginInfo (val uri: URI, val credentials: Credentials)

    private fun queryPrincipalUrl(context: Context, loginInfo: LoginInfo): String? {
        val log = java.util.logging.Logger.getLogger("peoplesync.PrincipalFinder")

        val httpClient: HttpClient = HttpClient.Builder(context, logger = log)
    ...
    private fun getPrincipalUrl(context: Context, parent: Account, loginInfo: LoginInfo): String? {
        ...
        val principalUrl = queryPrincipalUrl(context, loginInfo)
    ...
    @Synchronized
    fun upgradeAccounts(context: Context) {
        Logger.log.fine("Upgrading old accounts")
        val accountManager = AccountManager.get(context)
        ...
        if (showNotification) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (!powerManager.isIgnoringBatteryOptimizations(BuildConfig.APPLICATION_ID))
                    builder.addAction(0, context.getString(R.string.update_notification_action), pendingIntent)
                            .setDefaults(Notification.DEFAULT_SOUND)
            }
            val notify = builder.build()
            NotificationManagerCompat.from(context)
                    .notifyIfPossible(null, NotificationUtils.NOTIFY_UPDATE, notify)
        }
